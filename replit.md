...
- **Search**: Fast search with batch ingestion, sub-second responses using caching, cross-filter support, and deduplication based on unique identifiers and quality filtering. Implemented intelligent image-priority sorting that permanently ensures listings with valid images always appear before placeholder/no-image listings, regardless of user-selected sort order (price, year, mileage). The `hasValidImages()` helper detects placeholder patterns (placehold.co, spacer, no-image) to provide optimal user experience.
...
- **Project AETHER**: Adaptive Engine for Trust, Heuristics & Evolving Rankings - Production-ready SEO/GEO monitoring and analytics system at `/admin/aether`. Features: (1) GEO Sweep Monitoring - queries OpenAI with prompts to track CarArth mentions in AI responses, detects competitors (CarDekho, Cars24, Spinny, OLX), calculates mention rate and API costs, with batch sweep functionality and historical tracking. (2) SEO Audit Tool - analyzes any website's technical SEO health by checking sitemap.xml (including sitemap indexes), robots.txt, meta tags, canonical URLs, Schema.org markup, Open Graph tags, and heading structure; generates 0-100 score with severity-weighted issues (critical=-10, medium=-5, low=-2) and actionable recommendations; extracts sample URLs from sitemap for smart page auditing. (3) Content Brief Generator - uses OpenAI to create SEO-optimized content briefs with 15-20 India-specific trending keywords (long-tail, city-specific, brand-specific, feature-focused), complete content outlines, meta descriptions, and recommended word counts (1500-2500 words). (4) **Competitive Benchmarking & Playbooks** - Production-ready competitive intelligence system tracking CarArth's performance against 6 major competitors (CarDekho, Cars24, Spinny, OLX, CarWale, CarTrade) across 5 SEO/GEO pillars (Technical SEO, Content Quality, Schema/Structured Data, AI Visibility, User Experience). Features 4 ingestion workers (crawlSnapshot for sitemap parsing and Lighthouse metrics, aiVisibilityProbe for GEO mention tracking, scoreBenchmarks for AETHER weight-based scoring, recommendations for playbook generation), 25+ comprehensive benchmarking rules with KPI thresholds, API routes at `/api/aether/bench/*` with RBAC, React UI with score cards/comparison table/gap analysis, and nightly scheduler (cron 0 2 * * *) controllable via `AETHER_BENCHMARK_CRON_ENABLED` env var. All features include admin authentication, Zod validation, database persistence with PostgreSQL, and real-time dashboard UI with TanStack Query.
...
### LLM Providers
- **OpenAI**: For ToS extraction and normalization, AETHER GEO monitoring, and content brief generation.
