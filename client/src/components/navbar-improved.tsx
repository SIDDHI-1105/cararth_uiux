...
export default function NavbarImproved() {
  ...
  const navStyle: React.CSSProperties = {
    ...
    borderBottom: isDark ? `1px solid var(--gray-800)` : `1px solid var(--gray-100)`,
  ...
  const linkStyle = (isActive: boolean, color?: string): React.CSSProperties => ({
    padding: '8px 16px',
    cursor: 'pointer',
    color: isActive ? 'var(--white)' : (isDark ? 'var(--white)' : 'var(--foreground)'),
    backgroundColor: isActive ? (color || 'var(--brand)') : 'transparent',
    borderRadius: '8px',
    fontWeight: 600,
    fontSize: '15px',
    textDecoration: 'none',
  ...
  const mobileMenuStyle: React.CSSProperties = {
    ...
    right: 0,
    bottom: 0,
    backgroundColor: isDark ? 'rgba(11,29,83,0.95)' : 'rgba(255,255,255,0.98)',
